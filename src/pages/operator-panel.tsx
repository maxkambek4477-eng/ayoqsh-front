...
export default function OperatorPanel() {
  ...
  const autoPrint = useCallback(
    (check: Check) => {
      ...
      const html = `<!DOCTYPE html><html><head><title>Chek</title><style>body{font-family:Arial;text-align:center;padding:20px;margin:0}.check{border:2px solid #000;padding:20px;width:280px;margin:auto}.title{font-size:32px;font-weight:bold;margin:0}.station{font-size:14px;color:#333;margin:5px 0 15px 0}.code{font-size:28px;font-weight:bold;font-family:monospace;letter-spacing:2px;margin:10px 0}.litr{font-size:32px;font-weight:bold;color:#0066cc;margin:10px 0}.label{font-size:14px;color:#666;margin:5px 0}.contact{font-size:11px;color:#666;margin-top:15px;border-top:1px dashed #ccc;padding-top:10px}</style></head><body><div class="check"><h1 class="title">NBS GAZ OIL</h1><p class="station">Avtomobillarga Yoqilg'i Quyish Shahobchasi aksiya cheki</p><p style="font-size:15px;color:#333;margin:10px 0">Chekni quyidagi bot orqali ro'yxatdan o'tkazing<br/><br/>@NBS_GAZ_OIL_BOT</p>${qrImg}<p class="code">${check.code}</p><p class="label">Chek kodi</p><p class="litr">${check.amountLiters} L</p><p style="font-size:12px;color:#666">${format(new Date(), "dd.MM.yyyy HH:mm")}</p><p class="contact">Murojaat va qo'shimcha ma'lumot uchun:<br/><br/>telegram: @nbs_gaz_oil<br/><br/>tel: +998 90 600 44 77</p></div><script>window.onafterprint=function(){window.close();};window.print();</script></body></html>`;
